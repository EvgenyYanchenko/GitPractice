...
# Состояния в файлах Git

## Tracked & Untracked

 Файлы попадая в репозиторий получают статус Untracked - неотслеживаемые. Про них Гит знает, и постоянно нам об этом говорит, но не следит за их историей изменения. Это мы можем проверить коммандой 
git status
Чтобы нам следить за изменениями в файлах, нужно их добавить для отслеживания командой
git add filename
С этого момента Гит следит за файлом и его изменениями. Файл в состоянии Tracked & Staged(отслеживаемый, подготовлен для коммита). Если на этом этапе внести изменения в него,  в гит будут 2 версии файла: предыдущая - Tracked & Staged(отслеживаемый, подготовлен для коммита) и новая Tracked & Modified(отслеживаемый, измененная, НЕ подготовлен для коммита). На этом этапе можно решить - добавлять изменения в коммит или нет с помо git add filename.

Далее в коммит попадут только Tracked & Staged файлы. Для этого нужно ввести
git commit -m 'SOme text for descriptions about commit'

Теперь команда 
git status  
скажет что нет изменений для коммита.
